# -*- coding: utf-8 -*-
"""
@Time ： 2020/2/27 4:17 下午
@Auth ： Robbie Deng
@File ：ResNetPlus_edit.py
"""
...
def get_res_layer(output):
    ...
    dense_res21 = Dense(20, activation='selu', kernel_initializer='lecun_normal')(dense_res12)
    dense_res22 = Dense(24, activation='linear', kernel_initializer='lecun_normal')(dense_res21)

    dense_res31 = Dense(20, activation='selu', kernel_initializer='lecun_normal')(dense_res22)
    dense_res32 = Dense(24, activation='linear', kernel_initializer='lecun_normal')(dense_res31)

    dense_res41 = Dense(20, activation='selu', kernel_initializer='lecun_normal')(dense_res32)
    dense_res42 = Dense(24, activation='linear', kernel_initializer='lecun_normal')(dense_res41)

    dense_res51 = Dense(20, activation='selu', kernel_initializer='lecun_normal')(dense_res42)
    dense_res52 = Dense(24, activation='linear', kernel_initializer='lecun_normal')(dense_res51)

    output_new = average([dense_res52, output])
...
# resnet残差块一共有30个
resnet_output1 = get_res_layer(output_pre)
resnet_output2 = get_res_layer(resnet_output1)
resnet_output3 = get_res_layer(resnet_output2)
resnet_output4 = get_res_layer(resnet_output3)
resnet_output5 = get_res_layer(resnet_output4)
resnet_output6 = get_res_layer2(output_pre,resnet_output5)

output=output_pre
output2 = resnet_output6
...
model2 = get_model2()
model2.compile(optimizer='adam', loss=penalized_loss)
...
if TRAIN:
    ...
    for i in range(NUM_REPEAT):
        ...
        shuffle_weights(model2)

        history_1 = model2.fit(X_train_fit, Y_train_fit,
                              epochs=1200, batch_size=BATCH_SIZE, validation_data=None)

        model2.save_weights('complete' + str(i + 1) + '1_weights_2.h5')
        pred_1 = model2.predict(X_test_pred)
        pred_eval_1 = pred_1.reshape(24 * NUM_TEST_DAYS)

        history_2 = model2.fit(X_train_fit, Y_train_fit,
                              epochs=50, batch_size=BATCH_SIZE, validation_data=None)

        model2.save_weights('complete' + str(i + 1) + '2_weights_2.h5')
        pred_2 = model2.predict(X_test_pred)
        pred_eval_2 = pred_2.reshape(24 * NUM_TEST_DAYS)

        history_3 = model2.fit(X_train_fit, Y_train_fit,
                              epochs=50, batch_size=BATCH_SIZE, validation_data=None)

        model2.save_weights('complete' + str(i + 1) + '3_weights_2.h5')
        pred_3 = model2.predict(X_test_pred)
        pred_eval_3 = pred_3.reshape(24 * NUM_TEST_DAYS)

        history_4 = model2.fit(X_train_fit, Y_train_fit,
                              epochs=50, batch_size=BATCH_SIZE, validation_data=None)

        model2.save_weights('complete' + str(i + 1) + '4_weights_2.h5')
        pred_4 = model2.predict(X_test_pred)
        pred_eval_4 = pred_4.reshape(24 * NUM_TEST_DAYS)
...
mape2 = np.mean(np.divide(np.abs(Y_test_eval - pred_eval2), Y_test_eval))
mae2 = np.mean(np.abs(Y_test_eval - pred_eval2))
